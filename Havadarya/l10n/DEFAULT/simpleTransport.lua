simpleTransport = {}
simpleTransport.transportPilotNames = {"Black 4-1", "Black 4-2", "Black 4-3", "Black 4-4",
                                       "Green 3-1", "Green 3-2", "Green 3-3", "Green 3-4", "HQ 1-1",
                                       "HQ 1-2"}
simpleTransport.pickupGroupNames = {"AT Plt Helo 1", "AT Plt Helo 2", "AT Plt Helo 3",
                                    "AT Plt Helo 4", "AT Plt Helo 5", "AT Plt Helo 6",
                                    "Blue Mortar-1", "Blue Mortar-2"}
simpleTransport.pickupGroups = {}

simpleTransport.unitWeights = {
...
local getWeight = function(unit)
    local type = unit:getTypeName()
    for typename, weight in ipairs(simpleTransport.unitWeights) do
        if _typename == type then
            return weight
        end
    end
    -- default to 200 kg (?)
...
local loadGroup = function(playerUnitName)
    ...
    local details = simpleTransport.transportedUnits:add(playerUnit, nearestGroup)
    trigger.action.setUnitInternalCargo(playerUnitName, details["weight"])
...
local unloadGroup = function(playerUnitName)
    ...
    if isInAir(playerUnit) then
        sendMsgToUnit(playerUnit, "You must come to a stop on the ground before unloading troops.")
        return
    end
    ...
    trigger.action.setUnitInternalCargo(playerUnitName, 0)

    local playerPos = playerUnit:getPosition()
    spawnGroup(playerUnit, playerPos, groupDetails)
    local s = _sf("Group %s deployed.", groupDetails['groupName'])
    sendMsgToUnit(playerUnit, s)
end
